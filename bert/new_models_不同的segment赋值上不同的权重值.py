...
class  new_Bert(tf.keras.layers.Layer):
    name = 'bert'
    def __init__(self,
                maxlen = 128,#0
    ...
    def build(self, input_ids):
        ...
        if isinstance(input_ids,list):
            assert len(input_ids) == 3,"输入的list的长度必须为3，以[input_ids,segment_ids,label_ids]的形式进行输入"
            input_ids_shape,token_type_ids_shape,label_ids_shape = input_ids
        else:
        ...
        super(new_Bert, self).build(input_ids)
...
class  Embeddings(tf.keras.layers.Layer):
    ...
    def build(self, input_ids):
        ...
        super(Embeddings,self).build(input_ids)
    def call(self,input_ids):
        ...
        segment_ids = None
        if isinstance(input_ids,list):
            assert 3 == len(input_ids),"Expecting inputs to be a [input_ids,segment_ids,label_ids] list"
            input_ids,segment_ids,label_ids = input_ids[0],input_ids[1],input_ids[2]
        ...
        #print('results1 = ')
        #print(results)
        #print('after add segment_embeddings')
        #print('results = ')
        #print(results)
        label_ids = tf.expand_dims(label_ids,-1)
        results = results+label_ids
        #print('after add label_ids')
        #print('results = ')
        #print(results)
