...
class  Bert(tf.keras.layers.Layer):
    ...
    def call(self,input_ids):
        ...
        assert judge1 or judge2, "Expecting input to be a tensor or numpy type"
...
class  Transformer(tf.keras.layers.Layer):
    ...
    def build(self,input_shape):
        ...
        self.layer_norm0 = LayerNormalization(name = "layer_normalization_0")
        ...
        self.layer_norm1 = LayerNormalization(name = "layer_normalization_1")
    ...
    def call(self,inputs,**kwargs):
        ...
        embedding_output = self.layer_norm0(embedding_output)
        ...
        embedding_output = self.layer_norm1(embedding_output)
...
class AttentionLayer(tf.keras.layers.Layer):
    ...
    def create_attention_mask(from_shape, input_mask):
        mask = tf.cast(tf.expand_dims(input_mask,axis=1),tf.float32)
        #mask = (None,1,128),切掉最后一个维度，只留下相应的编号,[batch_size,1,
        ones = tf.expand_dims(tf.ones(shape=from_shape[:2],dtype=tf.float32),axis=-1)
        #ones = (None,128,1),为全1矩阵
        mask = ones*mask
        return mask#对应内容[B,F,T]
