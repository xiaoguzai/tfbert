...
def load_stock_weights(bert=Bert,new_tokens=[],ckpt_path=''):
    ...
    for layer_ndx in range(bert.num_layers):
        ...
        #这里bert/mlm_dense1/bias:0不匹配：Skipping weight:[bert/mlm_dense1/bias:0] as the weight
        #shape:[(13574,)] is not compatible with the checkpoint:[cls/predictions/output_bias]
        #shape:(21128,)
    weight_value_tuples = []
    loaded_weights = set()
    ...
    for ndx, (param_value, param) in enumerate(zip(param_values,bert_params)):
        ...
        if ckpt_reader.has_tensor(stock_name):
            ...
            #type ckpt_value = <class 'numpy.ndarray'>
            if param.name == 'bert/mlm_dense1/kernel:0':
                if len(new_tokens) == 0:
                    ckpt_value = ckpt_value.transpose()
                else:
                    current_value = np.array([ckpt_value[data] for data in new_tokens])
                    #current_value.shape = (13584,768)
                    ckpt_value = current_value.transpose()
            elif param.name == 'bert/embeddings/word_embeddings/embeddings:0' or param.name == 'bert/mlm_dense1/bias:0':
                #注意这里一定要写成elif,否则会发生报错
                if len(new_tokens) == 0:
                    pass
                else:
                    current_value = np.array([ckpt_value[data] for data in new_tokens])
                    ckpt_value = current_value
